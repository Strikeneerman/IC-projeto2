...
// Modified encode function to handle both intra and inter frames with custom parameters
void encodeRawVideo(const string& inputFile, const string& outputFile, 
                    int m, const BlockMatchingParams& params = BlockMatchingParams()) {
    ...
    while (input.read(reinterpret_cast<char*>(currentFrame.data), frameSize)) {
        // Write frame type (0 for intra, 1 for inter)
        ...
        if (isIntra) {
            ...
            currentFrame.copyTo(referenceFrame);  // Save for future inter frames
        } else {
...
// Modified decode function to handle both intra and inter frames with custom parameters
